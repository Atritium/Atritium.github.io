...
- 帧同步
  - 当网络卡顿时，整个游戏世界会“冻结”或“瞬移”，因为客户端必须等待所有人的操作指令到齐才能推进下一帧逻辑，如果没收到指令，逻辑就会停下
  - 如果是乐观帧同步（也采用了预测的帧同步）
    - 体感和状态同步相近，但当本地预测和服务器最终裁定结果不同时，表现会有微妙差别
    - 乐观帧同步在发生结果不一致时，会进行回滚，它会回退到上一个确定的逻辑帧（画面会发生闪回或者跳变），重新执行所有指令序列。所以如果看到时间倒流又极速快进的表现，一般是乐观帧同步
    - 状态同步则会进行直接进行校正，服务器直接发来“正确的位置”，客户端强制拉回或平滑过渡
- 状态同步
  - 当你卡顿时，你可能会发现你还能走动，但周围的怪物在原地跑路，或者你的位置被服务器强制“拉回”。这是因为客户端在本地先行模拟了逻辑，但服务器随后下发了权威的状态校正
